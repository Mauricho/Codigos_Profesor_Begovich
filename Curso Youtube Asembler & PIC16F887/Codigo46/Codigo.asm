...
; tecla deberá interrumpir y transmitir por UART a 9600 Baudios el número de tecla que se apretó.
; Se le agregó al enviar la palabra 'Hola' antes del valor. 	    
; Al recibir un dato por UART deberá interrumpir y mostrar el dato por LEDs conectados a los pines de
; Puerto A (parte alta). Los valores que se reciben van desde el caracter 'a' en adelante.
; 	    
; Además de asignar una velocidad correspondiente a un motor de CD manejado por el PWM.
; 	    
...
;   Configuramos el Timer0 con Option_Reg
	    ...
	    MOVLW		.250		;Con prescaler del Timer2=4 se obtiene una Fpwm=1[KHz]
	    MOVWF		PR2
...
TX_UART	    TX_LETRA_UART	'H'
	    ...
	    ADDLW               0X30		 ;Lo envía en ASCII   
	    MOVWF		TXREG		;Escribimos para que se transmitan los datos 
	    
	    TX_LETRA_UART	.10		;Cargo un valor para transmitir, en este caso es el salto de línea
	    
	    TX_LETRA_UART	.13		;Cargo un valor para transmitir, en este caso es el retorno de carro (volver al principio de línea)
...
;Algoritmo para convertir los números que se pulsan en el teclado 4x4 a ASCII   
	    MOVF		RCREG,W		;'a'=0x61
	    
	    MOVWF		0X30		;Enviamos el dato recibido a un registro de la RAM
	    MOVLW		0X60		;0x62-0x60=0x02 --> swap(0x02)= 0x20 lo que se asigna al puerto A
	    SUBWF		0X30,F
	    
	    SWAPF		0X30,W
	    ANDLW		0XF0		;Eliminamos la parte baja
;**********************************************************************	    
	    MOVWF		PORTA		 ;Sacamos el valor por el puerto A: 0x10 0x20 0x30 0x40 ... <-- a b c d ...
	    MOVWF		CCPR1L		 ;Controlamos la velocidad del motor mediante la señal PWM
...
;********************** Subrutina para transmitir caracteres **********
TX_CARACT_UART
; Pregunto si puedo transmitir: 
	    BSF			STATUS,RP0	;Banco 1   
	    BTFSS		TXSTA,TRMT	;Antes de transmitir pregunto si es que puedo.
	    GOTO		    $-1		;Pregunto hasta que pueda transmitir
	    BCF			STATUS,RP0	;Banco 0
	    
	    MOVWF		TXREG		;Cargo el valor de W al registro TXREG para que sea transmitido
